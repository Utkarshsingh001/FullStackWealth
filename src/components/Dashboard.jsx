...
import { TrendingUp, TrendingDown, Wallet, PieChart, AlertCircle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div>
        <h2 className="text-2xl font-semibold text-slate-800 dark:text-white">Dashboard</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-5 border border-gray-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Monthly Cash Flow</p>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
            <div className="flex items-center justify-between">
              <p className="text-xs text-slate-500 dark:text-slate-400">Debt-to-Income Ratio</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-5 border border-gray-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Portfolio Performance</p>
              <h3 className={`mt-1 text-xl font-semibold ${
                getGainLoss() >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'
              }`}>
              ...
              <div key={type} className="flex items-center justify-between">
                <div className="flex items-center">
                  <div className={`w-2 h-2 rounded-full mr-2 ${
                    type === 'FD' ? 'bg-blue-500' :
                    type === 'Mutual Fund' ? 'bg-purple-500' :
                    type === 'Stocks' ? 'bg-emerald-500' :
                    type === 'Gold' ? 'bg-amber-500' :
                    type === 'Property' ? 'bg-indigo-500' :
          ...
          <div className="space-y-3">
            {Object.entries(liabilityBreakdown).map(([type, value]) => (
              <div key={type} className="flex items-center justify-between">
                <div className="flex items-center">
                  <div className={`w-2 h-2 rounded-full mr-2 ${
                    type === 'Credit Card' ? 'bg-purple-500' :
                    type === 'Personal Loan' ? 'bg-blue-500' :
                    type === 'Home Loan' ? 'bg-emerald-500' :
                    type === 'Auto Loan' ? 'bg-orange-500' :
                    type === 'Student Loan' ? 'bg-indigo-500' :
                    'bg-gray-500'
                  }`} />
                  <span className="text-sm text-slate-600 dark:text-slate-400">{type}</span>
                </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
        <div className="p-5 border-b border-gray-200 dark:border-slate-700">
          <h3 className="text-lg font-medium text-slate-800 dark:text-white">Recent Assets</h3>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
        <div className="p-5 border-b border-gray-200 dark:border-slate-700">
          <h3 className="text-lg font-medium text-slate-800 dark:text-white">Recent Liabilities</h3>
        </div>
        <div className="divide-y divide-gray-200 dark:divide-slate-700">
