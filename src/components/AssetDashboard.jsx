...
const AssetDashboard = () => {
  ...
  const handleExpandAsset = (index) => {
    setExpandedAssetIndex(expandedAssetIndex === index ? null : index);
  };

  const existingTypes = ['All', ...new Set(assets.map(a => a.type))];
      ...
      <div className={viewMode === 'cards' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'}>
        {filteredAssets.map((asset, index) => (
          <AssetCard 
            ...
            onExpand={() => handleExpandAsset(index)}
          />
        ))}
