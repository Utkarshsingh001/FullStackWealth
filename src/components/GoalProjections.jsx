  ...
  const calculateProjections = () => {
    ...
    const projections = years.map(year => {
      ...
      const yearlyGoals = goals.filter(goal => 
        new Date(goal.targetDate).getFullYear() === year
      ).reduce((sum, goal) => sum + goal.targetAmount, 0);

      // Apply inflation to goals
      const inflatedGoals = yearlyGoals * Math.pow(1 + inflationRate, year - new Date().getFullYear());
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={index} className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-5 border border-slate-200 dark:border-slate-700">
            <p className="text-sm text-slate-500 dark:text-slate-400">{stat.label}</p>
            <p className={`mt-2 text-2xl font-semibold ${getColorClasses(stat.color)}`}>
