  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-800 dark:text-white">Liabilities</h2>
      ...
      {getUpcomingPayments().length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/50 
          ...
          <div>
            ...
            <ul className="mt-2 text-sm text-amber-700 dark:text-amber-400 space-y-1">
              {getUpcomingPayments().map((liability, index) => (
                <li key={index}>
                  {liability.name} - ₹{Number(liability.monthlyInstallment).toLocaleString()} due on{' '}
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 border border-slate-200 dark:border-slate-700 animate-slideDown">
          <LiabilityForm 
            ...
            onSave={(liability) => {
              ...
              updateLiability(liabilities.length - 1, 'principal', liability.principal);
              updateLiability(liabilities.length - 1, 'interestRate', liability.interestRate);
              updateLiability(liabilities.length - 1, 'tenure', liability.tenure);
              updateLiability(liabilities.length - 1, 'monthlyInstallment', liability.monthlyInstallment);
              updateLiability(liabilities.length - 1, 'outstandingAmount', liability.outstandingAmount);
              updateLiability(liabilities.length - 1, 'lender', liability.lender);
              updateLiability(liabilities.length - 1, 'nextPaymentDate', liability.nextPaymentDate);
              updateLiability(liabilities.length - 1, 'dueDate', liability.dueDate);
              updateLiability(liabilities.length - 1, 'isActive', liability.isActive);
